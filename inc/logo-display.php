...
function fau_get_svg_path($name, $echo = true) {
    ...
    if ($name === 'fau-logo-2021') {
        ...
        } elseif ($website_type === 'fau' && !is_front_page()) {
            // The faue_fau_logo_color setting only applies to the homepage; subpages always use the white logo
...
function fau_elemental_display_logo_title() {
    ...
    if (faue_is_cooperation_website($website_type) && has_custom_logo()) {
        ...
        if ($custom_logo) {
            echo '<img src="' . esc_url($custom_logo[0]) . '" alt="' . esc_attr(get_bloginfo('name')) . '" class="custom-logo">';
        } else {
            the_custom_logo();
        }
        echo '</span>';
