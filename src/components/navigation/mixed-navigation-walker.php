...
class Mixed_Navigation_Walker extends Walker_Nav_Menu {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
        ...
        $link_attributes .= ' class="menu-item-link"';
        ...
        if ($has_children) {
            $toggle_class = 'menu-modal__submenu-toggle';
            
            // Add specific classes based on child types
            if ($has_menu_children && $has_page_children) {
                $toggle_class .= ' mixed-toggle';
            } else if ($has_page_children) {
                $toggle_class .= ' page-toggle';
            } else {
                $toggle_class .= ' menu-toggle';
            }
            
            $output .= '<button class="' . esc_attr($toggle_class) . '" ';
            $output .= 'aria-expanded="false" ';
            $output .= 'aria-label="' . esc_attr(sprintf(__('Expand %s submenu', 'fau-elemental'), $item->title)) . '" ';
            $output .= 'data-menu-children="' . esc_attr(count($menu_children)) . '" ';
            $output .= 'data-page-children="' . esc_attr(count($page_children)) . '">';
            
            // Add appropriate icon based on child types
            $output .= '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">';
            $output .= '<path d="M7 5L12 10L7 15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
            $output .= '</svg>';
    ...
    public function end_el(&$output, $item, $depth = 0, $args = null) {
        // Only add page children if we're not already inside a submenu created by menu children
        // This prevents duplicate page children when they're already added by the walker
    ...
    private function get_menu_children($item) {
        ...
        if (empty($this->all_menu_items)) {
            ...
            foreach (array('primary', 'meta_navigation_services', 'meta_navigation_structure') as $location) {
    ...
    private function add_page_children_to_output(&$output, $page_children, $depth = 1) {
        ...
        foreach ($page_children as $page) {
            ...
            // Add toggle button if page has children
            if ($has_children) {
                $output .= '<button class="menu-modal__submenu-toggle page-toggle" ';
                $output .= 'aria-expanded="false" ';
                $output .= 'aria-label="' . esc_attr(sprintf(__('Expand %s submenu', 'fau-elemental'), $page->post_title)) . '" ';
                ...
                // Add child pages
                $output .= '<ul class="sub-menu page-submenu" data-depth="' . esc_attr($depth) . '">';
