...
class Menu_Modal {
    ...
    private function build_menu_html($menu_items, $menu_class, $walker_class = null) {
        ...
        // Create args object to pass menu items to walker
        $args = (object) array(
     ...
     * @param object $args The args object with menu items
     */
    private function build_menu_items_recursive($menu_items, &$output, $walker, $parent_id = 0, $depth = 0, $args = null) {
        ...
        foreach ($current_level_items as $item) {
            // Check if this item has children (menu children or page children)
            $has_menu_children = false;
            foreach ($menu_items as $potential_child) {
                if ($potential_child->menu_item_parent == $item->ID) {
                    $has_menu_children = true;
                    break;
                }
            }
            
            // For Mixed Navigation Walker, also check for page children
            $has_page_children = false;
            if ($walker instanceof Mixed_Navigation_Walker) {
    ...
    private function render_menu_content($modal_id, $config) {
        ...
        foreach ($theme_locations as $location) {
            ...
            if (has_nav_menu($location)) {
                // For mixed navigation walker, we need to get menu items and pass them
                if ($walker_class === 'Mixed_Navigation_Walker') {
                    $this->render_local_menu_with_mixed_navigation($location, $menu_class, $depth, $walker_class);
                } else {
    ...
    /**
     * Render local menu with mixed navigation support
     *
     * @param string $location Theme location
     * @param string $menu_class CSS class for menu
     * @param int $depth Menu depth
     * @param string $walker_class Walker class name
     */
    private function render_local_menu_with_mixed_navigation($location, $menu_class, $depth, $walker_class) {
        $locations = get_nav_menu_locations();
        ...
        // Use our custom build method for mixed navigation
        echo $this->build_menu_html($menu_items, $menu_class . ' ' . $menu_class . '--' . str_replace('_', '-', $location), $walker_class);
    }
