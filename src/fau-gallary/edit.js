...
export default function Edit({ attributes, setAttributes }) {
	...
	const addGalleryItem = () => {
		const newItems = [...(galleryItems || []), { imageUrl: '', caption: '', copyright: '' }];
	...
	return (
		<div {...useBlockProps()}>
			<InspectorControls>
				<PanelBody title={__('Gallery Items', 'fau-gallery')} initialOpen={true}>
					{(galleryItems || []).map((item, index) => (
						<div key={index} className="gallery-item-settings">
							<h4>{__('Gallery Item', 'fau-gallery')} {index + 1}</h4>
							<MediaUpload
								...
								render={({ open }) => (
									<Button onClick={open} isSecondary>
										{item.imageUrl ? (
											<img
												src={item.imageUrl}
												alt={__('Gallery Item Image', 'fau-gallery')}
							...
							/>
							<TextControl
								label={__('Copyright Notice', 'fau-gallery')}
					...
					<div key={index} className="gallery-item" style={{ position: 'relative' }}>
						{item.imageUrl && (
							<div style={{ position: 'relative' }}>
								<img src={item.imageUrl} alt={__('Gallery Item', 'fau-gallery')} style={{ width: '100%', height: 'auto' }} />
								<button
									...
									onClick={() => {
										...
										fullscreenWindow.document.title = __('Fullscreen Image', 'fau-gallery');
									}}
									style={{
										position: 'absolute',
										top: '8px',
