...
export default function Edit({ attributes, setAttributes }) {
	...
	const { availablePages, pages, allPages } = useSelect(
		(select) => {
			...
			const pagesData = getEntityRecords('postType', 'page', query) || [];
			const selectedPagesData = selectedPages.length > 0 
	...
	const blockProps = useBlockProps({
		className: `fau-big-button-teaser-group fau-big-button-teaser-group--${teaserSize} fau-big-button-teaser-group--${variant} ${facultyColor !== 'default' ? `fau-big-button-teaser-group--${facultyColor}` : ''}`,
	});
	...
	const removeSelectedPage = (pageId) => {
		const newSelectedPages = selectedPages.filter(p => p.id !== pageId);
		setAttributes({ selectedPages: newSelectedPages });
	};
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Content Settings', 'fau-elemental')} initialOpen={true}>
					<ToggleControl
					...
					<ToggleControl
						...
						onChange={(value) => setAttributes({ showTeaserText: value })}
					/>
					{showTeaserText && (
						<TextareaControl
				...
				<PanelBody title={__('Appearance Settings', 'fau-elemental')} initialOpen={false}>
					<SelectControl
						...
						onChange={(value) => setAttributes({ facultyColor: value })}
					/>
					...
					<SelectControl
						...
						value={teaserSize}
						options={[
							{ label: __('Small', 'fau-elemental'), value: 'small' },
							{ label: __('Large', 'fau-elemental'), value: 'large' },
						]}
						onChange={(value) => setAttributes({ teaserSize: value })}
					/>
					...
					<SelectControl
						...
						value={variant}
						options={[
							{ label: __('Filled', 'fau-elemental'), value: 'filled' },
							{ label: __('Outline', 'fau-elemental'), value: 'outline' },
						]}
					...
					<RangeControl
						...
						min={1}
						help={selectedPages.length >= numberOfButtons ?
							__('Maximum number of pages selected for current button count.', 'fau-elemental') :
							__('Select the number of buttons to display.', 'fau-elemental')
						}
					/>
				</PanelBody>

				<PanelBody title={__('Page Selection', 'fau-elemental')} initialOpen={false}>
					<ToggleControl
						label={__('Show All Pages', 'fau-elemental')}
						checked={showAllPages}
						onChange={(value) => setAttributes({ showAllPages: value })}
						help={showAllPages ? 
							__('All published pages will be displayed as buttons.', 'fau-elemental') :
							__('Manually select specific pages to display as buttons.', 'fau-elemental')
						}
					/>
					...
					{!showAllPages && (
						<>
							<ComboboxControl
								label={__('Search and select pages', 'fau-elemental')}
								value=""
								...
								aria-label={__('Search and select pages', 'fau-elemental')}
								__next40pxDefaultSize={true}
								__nextHasNoMarginBottom={true}
								help={selectedPages.length >= numberOfButtons ? 
									__('Maximum number of pages selected for current button count.', 'fau-elemental') : 
									__('Type to search for pages to add as button teasers.', 'fau-elemental')
								}
							/>

							<div className="selected-pages-list">
								{selectedPages.map((page, index) => (
									<div key={page.id} className="selected-page-item" style={{ 
										marginBottom: '10px', 
										padding: '10px', 
										...
										<div style={{ marginBottom: '8px' }}>
											<strong>{__('Button', 'fau-elemental')} {index + 1}: </strong>
											<span dangerouslySetInnerHTML={{ __html: page.title }} />
										</div>
										<Button
											isSmall
											isDestructive
											onClick={() => removeSelectedPage(page.id)}
											aria-label={__('Remove page', 'fau-elemental') + ': ' + page.title}
										>
					...
					{showAllPages && allPages && (
						<div style={{ marginTop: '16px' }}>
							<p><strong>{__('Displaying all pages:', 'fau-elemental')}</strong></p>
							<div style={{ 
								maxHeight: '200px', 
								overflowY: 'auto', 
								border: '1px solid #ddd', 
								...
								{allPages.map((page, index) => (
									<div key={page.id} style={{ 
										...
										{index + 1}. <span dangerouslySetInnerHTML={{ __html: page.title.rendered }} />
									</div>
								))}
							</div>
							{allPages.length > numberOfButtons && (
								<p style={{ fontSize: '13px', color: '#d63638', marginTop: '8px' }}>
									{__('Note: Only the first ', 'fau-elemental') + numberOfButtons + __(' pages will be displayed based on your "Number of Buttons" setting.', 'fau-elemental')}
								</p>
							)}
						</div>
					)}
