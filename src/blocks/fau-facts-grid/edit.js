...
export default function Edit({ attributes, setAttributes }) {
	const { roofLine, showRoofLine, facts } = attributes;
	...
	return (
		<div {...blockProps}>
			<InspectorControls>
				<PanelBody title={__('Roof Line Settings', 'fau-elemental')}>
					<ToggleControl
				...
				<PanelBody title={__('Facts Management', 'fau-elemental')}>
					...
					<p style={{ fontSize: '0.9rem', color: '#666', margin: '0.5rem 0' }}>
						{__('Click on a fact in the editor to select and edit it.', 'fau-elemental')}
					</p>
				</PanelBody>

				{selectedFact && (
					<PanelBody title={__(`Fact ${selectedFactIndex + 1} Settings`, 'fau-elemental')}>
						<div style={{ marginBottom: '1rem' }}>
									...
									render={({ open }) => (
										<div>
											{selectedFact.iconUrl ? (
												<div style={{ marginBottom: '0.5rem' }}>
													<img
														...
														alt={__('Selected icon', 'fau-elemental')}
														style={{
															width: '60px',
															height: '60px',
															objectFit: 'contain',
											...
											<ButtonGroup>
												...
												>
													{selectedFact.iconUrl 
														? __('Change Icon', 'fau-elemental')
														: __('Upload Icon', 'fau-elemental')
													}
												</Button>
												{selectedFact.iconUrl && (
													<Button
						...
						<TextControl
							...
							onChange={(value) => updateFact(selectedFactIndex, 'link', value)}
							placeholder={__('https://example.com', 'fau-elemental')}
						/>

						<ToggleControl
							label={__('Show Link Button', 'fau-elemental')}
							checked={selectedFact.showLink}
							onChange={(value) => updateFact(selectedFactIndex, 'showLink', value)}
							help={selectedFact.link ? __('Toggle to show/hide the link button', 'fau-elemental') : __('Add a link URL above to enable this option', 'fau-elemental')}
						/>
					</PanelBody>
				)}
			</InspectorControls>
			...
			<div className="fau-facts-grid">
				{showRoofLine && (
					<RichText
						tagName="h3"
						className="fau-facts-grid-roof-line"
						value={roofLine}
				...
				<div className="fau-facts-grid-items">
					{facts.map((fact, index) => (
						<div
							key={index}
							className={`fau-facts-grid-item ${selectedFactIndex === index ? 'is-active' : ''} ${fact.link && fact.showLink ? 'has-link' : ''}`}
						...
						>
							...
							<div className="fau-facts-grid-item-content">
								<RichText
									...
									value={fact.text}
									onChange={(value) => updateFact(index, 'text', value)}
									placeholder={__('Enter fact text...', 'fau-elemental')}
								/>
								{fact.link && fact.showLink && (
									<div className="wp-block-buttons">
										<div className="wp-block-button is-style-tertiary">
											<a className="wp-block-button__link" href={fact.link}>
												{__('Mehr', 'fau-elemental')}
											</a>
										</div>
									</div>
								)}
				...
				{facts.length === 0 && (
					<div style={{ textAlign: 'center', padding: '2rem', color: '#666' }}>
						<p>{__('No facts added yet. Use the "Add New Fact" button in the sidebar to get started.', 'fau-elemental')}</p>
					</div>
				)}
			</div>
		</div>
