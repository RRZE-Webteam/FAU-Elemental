                        ...
                        if (!empty($search_query)) {
                            ...
                            if (!empty($results)) {
                                ...
                                foreach ($results as $result) {
                                    ...
                                    echo '<h3><a href="' . esc_url($result['link']) . '">' . esc_html($result['title']) . '</a></h3>';
                                    echo '<div class="search-result-meta">';
                                    echo '<span class="search-result-site">' . esc_html($result['blog_name']) . '</span>';
                                    echo '<span class="search-result-date">' . esc_html($result['date']) . '</span>';
                                    echo '<span class="search-result-type">' . esc_html($result['type']) . '</span>';
                                    echo '</div>';
                                    if (!empty($result['excerpt'])) {
                                        echo '<div class="search-result-excerpt">' . wp_kses_post($result['excerpt']) . '</div>';
                                    }
                                    echo '</article>';
                                }
                                echo '</div>';
