...
async function initializeTeaserGrids() {
	const teaserGrids = document.querySelectorAll(
		'.wp-block-fau-elemental-fau-list-item'
	);
...
async function initializeTeaserGrid( container ) {
		...
		try {
			...
			const posts = await Promise.all(
				selectedPosts.map( ( post ) =>
					fetch( `/wp-json/wp/v2/posts/${ post.id }?_embed` ).then(
						( r ) => r.json()
					)
				)
			);
			...
			grid.innerHTML = posts
				.map( ( post ) => renderPostTeaser( post, grid ) )
				.join( '' );
		} catch ( error ) {
	...
	try {
		...
		const totalItems =
			parseInt( countResponse.headers.get( 'X-WP-Total' ) ) || 0;

		// Calculate total pages
		const effectiveTotalPosts =
		...
		grid.innerHTML = items
			.map( ( item ) =>
				variant === 'post'
					? renderPostTeaser( item, grid )
					: renderPageTeaser( item, grid )
			)
			.join( '' );
	...
	return `
        <div class="teaser-item post-teaser">
            ...
            <div class="teaser-content-wrapper ${
				grid.classList.contains( 'is-style-dark' ) ? 'dark-theme' : ''
			}">
