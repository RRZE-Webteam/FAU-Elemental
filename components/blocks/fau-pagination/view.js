...
function initializePaginationBlock( paginationElement ) {
	...
	function attachClickHandlers() {
		...
		const existingHandlers = paginationElement.querySelectorAll(
		...
		const paginationControls = paginationElement.querySelectorAll(
			'.page-nav, .page-numbers a, .page-numbers span.page-number, .page-numbers button'
		);
...
function findAssociatedGrid( gridBlockId ) {
	...
	if ( ! grid ) {
		...
		grid = document.querySelector(
			`[data-custom-block-id="${ gridBlockId }"]`
		);
	}
...
function handlePaginationClick( e, gridContainer, paginationContainer ) {
	const clickedElement = e.target.closest(
		'a, button, span.page-number, .page-nav'
	);
	...
	if (
		...
		clickedElement.classList.contains( 'page-ellipsis' )
	) {
		return;
	}
	...
	const currentPage =
		...
		1;
	const totalPages =
		parseInt( paginationContainer.getAttribute( 'data-total-pages' ) ) || 1;
	...
	const isJsPagination = gridContainer.querySelector(
		'[data-js-pagination="true"]'
	);

	if ( isJsPagination ) {
		// Use JavaScript pagination
		const customBlockId = gridContainer.getAttribute(
			'data-custom-block-id'
		);
		if ( customBlockId ) {
			// Emit event for teaser grid to handle
			document.dispatchEvent(
				new CustomEvent( 'fau-pagination-change', {
					detail: {
...
function updateGridForPage( gridContainer, targetPage, paginationContainer ) {
	...
	const postsPerPage =
		gridContainer.getAttribute( 'data-posts-per-page' ) || '6';
	const displayStyle =
		gridContainer.getAttribute( 'data-display-style' ) || 'teaser-grid';
	const teaserLayout =
		gridContainer.getAttribute( 'data-teaser-layout' ) || '3m';
	const orderBy = gridContainer.getAttribute( 'data-order-by' ) || 'date';
	const order = gridContainer.getAttribute( 'data-order' ) || 'DESC';
	const headingLevel =
		gridContainer.getAttribute( 'data-heading-level' ) || 'h4';
...
function updatePaginationState( paginationContainer, currentPage ) {
	...
	const placeholderContainer = paginationContainer.querySelector(
		'.pagination-placeholder'
	);
	const controlsContainer = paginationContainer.querySelector(
		'.pagination-controls'
	);
...
function generateFullPaginationHTML( currentPage, totalPages ) {
	...
	if ( currentPage > 1 ) {
		html +=
			'<span class="page-nav prev" aria-label="Previous page"><span aria-hidden="true">‹</span></span>';
	} else {
		html +=
			'<span class="page-nav prev disabled" aria-disabled="true"><span aria-hidden="true">‹</span></span>';
	}
	...
	if ( currentPage < totalPages ) {
		html +=
			'<span class="page-nav next" aria-label="Next page"><span aria-hidden="true">›</span></span>';
	} else {
		html +=
			'<span class="page-nav next disabled" aria-disabled="true"><span aria-hidden="true">›</span></span>';
	}
